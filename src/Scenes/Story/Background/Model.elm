module Scenes.Story.Background.Model exposing
    ( initModel
    , updateModel
    , viewModel
    )
...
import Scenes.Story.Background.Common exposing (EnvC, Model, nullModel)
...
-- timeForward : Model -> Model
-- timeForward model =
--     if model.timer >= 3 then
--         { model | timer = 0, bgIndex = model.bgIndex + 1 }
--     else
--         model
