module Scenes.Level3.GameStuff.Model exposing
    ( initModel
    , updateModel
    , viewModel
    )
...
import MainConfig
import Scenes.Level3.GameStuff.Common exposing (EnvC, Model, SceneState(..), nullModel)
...
Add components here
-}
initModel : EnvC -> Level3Init -> Model
initModel env _ =
    nullModel env
...
updateModel env _ model =
    case env.msg of
        Tick _ ->
            if model.sceneState == End then
                ( model, [ ( LayerParentScene, LayerChangeSceneMsg "LogoEnd" ) ], env )

            else if model.sceneState == Beginning then
                ( { model | sceneState = FadeIn }, [ ( LayerParentScene, LayerSoundMsg "Level3Bgm" (getResourcePath "audio/level.ogg") ALoop ) ], env )
                ...
                let
                    ( newEnv, newModel ) =
                        ( env, model |> moveBall |> reflect |> movePaddle |> reflectDeadBorder )
                            ...
                            |> isFinishedFadeOut
                in
                ( newModel, [ ( LayerParentScene, LayerStopSoundMsg "Level3Bgm" ) ], newEnv )
...
moveBall model =
    { model
        | balls =
            List.map
                (\{ x, y, vx, vy, r, electricCharge, track } ->
                    let
                        dTheta =
                        ...
                        mtrack =
                            if List.length ntrack > 30 then
                                ...
                                ntrack
                    in
                    { x = x + vx, y = y + vy, vx = nvx, vy = nvy, r = r, electricCharge = electricCharge * 0.999, track = mtrack }
                )
                model.balls
    }
...
reflect model =
    let
        ...
        sballs =
            removeIntersectBallsPaddle ( mballs, mpaddle )
    in
    { model | balls = sballs, bricks = fbricks, paddle = mpaddle }
                ...
                case state of
                    Normal ->
                        renderSprite env.globalData
                            ...
                            -- "brick"
                            (if electricCharge > 0 then
                                "brick_positive"

                             else if electricCharge == 0 then
                                "brick_neutral"
