module Lib.Render.Text exposing
    ...
    , renderTextWithColorCenterMaxWidth
    )
...
import Canvas.Settings.Text exposing (TextAlign(..), TextBaseLine(..), align, baseLine, font, maxWidth)
...
renderTextWithColorCenterMaxWidth : GlobalData -> Int -> String -> String -> Color -> ( Int, Int ) -> Float -> Renderable
renderTextWithColorCenterMaxWidth gd size s ft col ( x, y ) mw =
    let
        rx =
            lengthToReal gd size
